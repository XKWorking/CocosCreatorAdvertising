...
export default class AudioManager extends cc.Component {
    ...
    static play(name: string, volume = 1, isLoop = false): void {
        ...
        if (audio) {
            AudioManager._playAudio(audio.audioClip, volume * audio.relativeVolume, isLoop)
        }
    }
    ...
    static pause(name: string): void {
        AudioManager._pauseAudio(AudioManager.getAudioClipByName(name))
    }
    ...
    static resume(name: string): void {
        AudioManager._resumeAudio(AudioManager.getAudioClipByName(name))
    }
     ...
     * @param volume
     */
    private static _playAudio(src: any, volume = 1, isLoop = false): void {
